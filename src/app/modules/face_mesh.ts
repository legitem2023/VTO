import * as faceMeshSolutionApi from './face_mesh_solution_api'
...
export interface Results {
  ...
  multiFaceLandmarks: faceMeshSolutionApi.NormalizedLandmarkList
}
...
[437, 343], [343, 399], [399, 437], [344, 360], [360, 440], [440, 344], [420, 437], [437, 456], [456, 420], [360, 420], [420, 363], [363, 360], [361, 401], [401, 288], [288, 361], [265, 372], [372, 353], [353, 265], [390, 339], [339, 249], [249, 390], [339, 448], [448, 255], [255, 339]]
 ...
 */
export class FaceMeshSolution {
  camera(arg0: any) {
    throw new Error('Method not implemented.')
  }
  private readonly solution: faceMeshSolutionApi.Solution
  private listener?: ResultsListener

  constructor(config?: FaceMeshConfig) {
    config = config || {}
    this.solution = new faceMeshSolutionApi.Solution({
      locateFile: config.locateFile,
      files: [
        'face_mesh_hokei_solution_packed_assets_loader.js',
        'face_mesh_hokei_solution_simd_wasm_bin.js',
        'face_mesh_hokei_solution_wasm_bin.js'
      ],
      graph: { url: 'face_mesh_hokei.binarypb' },
      onRegisterListeners: (attachFn: faceMeshSolutionApi.AttachListenerFn) => {
        const thiz = this
        ...
        attachFn(
          ['output_video', 'multi_face_landmarks', 'multi_face_geometry'],
          (results: faceMeshSolutionApi.ResultMap) => {
            if (thiz.listener) {
              thiz.listener({
                image: results['output_video'] as HTMLCanvasElement,
                multiFaceLandmarks: results['multi_face_landmarks'] as faceMeshSolutionApi.NormalizedLandmarkList,
                multiFaceGeometry: results['multi_face_geometry'] as any,
              })
            }
          },
        )
  ...
  async send(inputs: Inputs): Promise<void> {
    ...
    if (inputs.image) {
      const image = { image: inputs.image }
      const metadata = {
        width: inputs.image.videoWidth,
        height: inputs.image.videoHeight,
        timestampMs: performance.now(),
      }
      await this.solution.processFrame('input_video', metadata, image, inputs.smoothFaceIntensity)
    }
  }
  ...
  setOptions(options: Options): void {
    this.solution.setOptions(
      (options as unknown) as faceMeshSolutionApi.OptionList,
    )
  }
}
